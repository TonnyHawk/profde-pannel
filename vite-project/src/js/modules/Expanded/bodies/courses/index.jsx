...
export default function CoursesBody(properties) {
  ...
  let featuresElem = features.map((elem, index) => {
    let image = '';
    if (elem.photo !== '')
      image = <img src={serverUrl + elem.photo} className="mb-3" alt="" />;
    return (
      <>
        {image}
      ...
      <div class="container" ref={(c) => (funcs.rootElem = c)}>
        ...
        <div class="row">
          <div class="col d-flex justify-content-center">
            <h1 class="my-5">
              {props.info.mode === 'add' ? 'Додати' : 'Редагувати'} курс
            </h1>
          </div>
        </div>
        <div class="row">
          <div class="col">
            ...
            >
              ...
              <div className="mb-4">
                <label class="form-label">Короткі деталі курсу</label>
                <br />
                {featuresElem}
                <div
                  class="btn btn-success"
                  onClick={() => funcs.addField('features')}
                >
                  Додати особливість
                </div>
              </div>
