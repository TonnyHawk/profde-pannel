...
async function getCollection(add) {
  ...
  if (response.ok) {
    ...
    alert(
      'Ошибка подгрузки данних ' +
        add +
        ' (HTTP): ' +
        response.status +
        'Спробуйте перезагрузити сторінку. Якщо не допоможе - помилка на сервері'
    );
  }
  return result;
}
...
class App extends Component {
  ...
  changePage(name) {
    ...
    if (nameArr[0] === 'sort') {
      ...
      Promise.all(
        this.setState({ currentPage: name }, async () => {
  ...
  render() {
    ...
    items = items.map((human, ind) => {
      ...
      if (typeof human.media === 'undefined') {
        const link = serverUrl + human.photo;
      ...
      let descr = '';
      if (typeof human.owner !== 'undefined')
        descr = <p class="gall-item__descr">{human.owner}</p>;
      if (currentPage === 'gallery') {
        if (human.about !== '')
          descr = (
            <p class="gall-item__descr">{textElipsizer(human.about, 80)}</p>
          );
      }
      ...
      return (
        <div
          class="gall__item gall-item"
          onClick={() => this.selectHuman(human)}
        >
          <div class="gall-item__photo">{mediaThing}</div>
    ...
    if (currentPage.split('-')[0] === 'sort') {
      ...
      content = <div class="gall__bd">{items}</div>;
    }

    let expandedPageElem = expandedPage.state ? (
      <Expanded
        info={expandedPage}
        pageType={currentPage}
        human={selectedItem}
            ...
            <nav role="navigation" id="nav">
              <div id="menuToggle">
                ...
                {/* <!--
                     ...
                     --> */}
                <ul id="menu">
                  <a onClick={() => this.changePage('humans')}>
                    <li>Люди</li>
                  </a>
                  <a onClick={() => this.changePage('certificates')}>
                    <li>Сертифікати</li>
                  </a>
                  <a onClick={() => this.changePage('books')}>
                    <li>Книги</li>
                  </a>
                  <a onClick={() => this.changePage('courses')}>
                    <li>Курси</li>
                  </a>
                  <a onClick={() => this.changePage('gallery')}>
                    <li>Галерея</li>
                  </a>
                </ul>
              </div>
              ...
              <div class="gall__inner">
                <div className="row gall__hd">
                  <div className="col-12 col-md-6">
                    ...
                    <div
                      class="btn-group btn-group-lg btn-group-toggle mb-3 mb-md-0"
                      role="group"
                    ...
                    >
                      ...
                      <button
                        type="button"
                        class={`btn btn-primary ${
                          this.state.filter === 'Deutsch' ? 'active' : ''
                        }`}
                        onClick={() => this.toggleFilter('Deutsch')}
                      >
                        Deutsch
                      </button>
                      <button
                        type="button"
                        class={`btn btn-primary ${
                          this.state.filter === 'English' ? 'active' : ''
                        }`}
                        onClick={() => this.toggleFilter('English')}
                      >
                        English
                      </button>
                    </div>
                  ...
                  <div className="col-12 col-md-6">
                    <div className="row gall__search justify-content-start justify-content-md-end">
                      ...
                      <div className="col-auto">
                        ...
                        >
                          Додати <i class="bi bi-plus-square"></i>
                        </div>
                      </div>
                      <div className="col-auto">
                        <div
