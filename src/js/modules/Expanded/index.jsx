...
function strCapitalize(str) {
  return str
    .split(' ')
    .map((elem) => {
      // making each first letter a capital
      let newStr = elem;
      let letterIndex = 0;
      ...
      newStr =
        elem[letterIndex].toUpperCase() +
        elem.slice(letterIndex + 1, elem.length);
      return newStr;
    })
    .join(' ');
}
...
class Expanded extends Component {
  ...
  handleChange(e) {
    ...
    if (e.target.getAttribute('data-index')) {
      // meens that we deal with an array
      let index = e.target.getAttribute('data-index');
      if (e.target.getAttribute('data-name')) {
        // if we need to change select pair
  ...
  deleteField(e) {
    ...
    if (key === 'certificates' || key === 'video' || key === 'features')
      minNumOfElements = 0;
    this.setState((state) => {
  ...
  async sendData() {
    ...
    if (!validation.access) {
      alert(
        'Здається ви заповнили форму не до кінця. будь ласка заповніть поля підсвічені червоним'
      );
    } else {
      ...
      if (pageType === 'humans') {
        ...
        human.video.forEach((elem) => {
          ...
          if (file.size > 0) {
            // if we added a new video to the form
            let video = file;
            reqData.set(
              'video-' + elem.professor,
              video,
              `video-${elem.professor}.mp4`
            );
          }
        });
      ...
      try {
        ...
        if (file.size > 0) {
          // if we added a new photo to the form
          let photo = file;
      ...
      else if (this.props.info.mode === 'add') {
        option = '/add';
      }
  ...
  render() {
    ...
    switch (type) {
      case 'humans':
        return (
          <HumansBody state={this.state} props={this.props} funcs={this} />
        );
      case 'certificates':
        return (
          <CertificatesBody
      ...
      case 'courses':
        return (
          <CoursesBody state={this.state} props={this.props} funcs={this} />
        );
      case 'gallery':
        return (
          <GalleryBody state={this.state} props={this.props} funcs={this} />
        );
      default:
